...
def render_psa_figure(triplet_table: pd.DataFrame, signal_label: str, title_suffix: str, out_base: Path) -> None:
    ...
    for ax, spec in zip(axes.ravel(), plot_specs):
        ...
        x_plot = x[mask]
        y_plot = y[mask]
        bins = max(20, min(80, int(np.sqrt(max(x_plot.size, 1)))))
        x_edges = np.linspace(lo, hi, bins + 1)
        y_edges = np.linspace(lo, hi, bins + 1)
        hist, _, _ = np.histogram2d(x_plot, y_plot, bins=[x_edges, y_edges])
        x_idx = np.clip(np.searchsorted(x_edges, x_plot, side="right") - 1, 0, bins - 1)
        y_idx = np.clip(np.searchsorted(y_edges, y_plot, side="right") - 1, 0, bins - 1)
        point_density = hist[x_idx, y_idx] if x_plot.size else np.array([], dtype=float)
        artist = ax.scatter(
            x_plot,
            y_plot,
            s=8 if x_plot.size >= 500 else 12,
            c=point_density,
            cmap="turbo",
            vmin=1.0,
            vmax=max(float(point_density.max()), 1.0) if point_density.size else 1.0,
            alpha=0.8,
            linewidths=0,
            rasterized=True,
        )
        cbar = fig.colorbar(artist, ax=ax)
        cbar.set_label("Local count")
        ax.set_xlim(lo, hi)
        ax.set_ylim(lo, hi)
