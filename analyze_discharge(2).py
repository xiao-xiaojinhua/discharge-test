...
def make_psa_pair_table(events: pd.DataFrame) -> pd.DataFrame:
    if len(events) < 2:
        return pd.DataFrame()

    sort_cols = [col for col in ["time_s", "event_sec", "event_nsec", "event_index"] if col in events.columns]
    if sort_cols:
        ordered = events.sort_values(sort_cols, kind="mergesort").reset_index(drop=True)
    else:
        ordered = events.reset_index(drop=True)
    prev = ordered.iloc[:-1].reset_index(drop=True)
    curr = ordered.iloc[1:].reset_index(drop=True)
    ...
    time_prev = pd.to_numeric(series_or_nan(prev, "time_s"), errors="coerce")
    time_curr = pd.to_numeric(series_or_nan(curr, "time_s"), errors="coerce")
    table = pd.DataFrame(
        {
            "datetime_prev": series_or_nan(prev, "datetime"),
            "datetime_curr": series_or_nan(curr, "datetime"),
            "time_s_prev": time_prev,
            "time_s_curr": time_curr,
            "phase_deg_prev": series_or_nan(prev, "phase_deg"),
            "phase_deg_curr": series_or_nan(curr, "phase_deg"),
            "amplitude_prev_raw": series_or_nan(prev, "amplitude_abs_raw"),
            "amplitude_curr_raw": series_or_nan(curr, "amplitude_abs_raw"),
            "delta_t_s": time_curr - time_prev,
        }
    )
    return table.dropna(subset=["amplitude_prev_raw", "amplitude_curr_raw"]).reset_index(drop=True)


def render_psa_figure(pair_table: pd.DataFrame, signal_label: str, title_suffix: str, out_base: Path) -> None:
    if pair_table.empty:
        return

    x = pair_table["amplitude_prev_raw"].to_numpy(dtype=float)
    y = pair_table["amplitude_curr_raw"].to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=SMALL_FIGSIZE)
    if len(pair_table) >= 30:
        artist = ax.hexbin(x, y, gridsize=35, mincnt=1, cmap="viridis")
        cbar_label = "Count"
    else:
        artist = ax.scatter(x, y, s=10, c=np.ones_like(x), cmap="viridis", vmin=0, vmax=max(len(pair_table), 1), alpha=0.75)
        cbar_label = "Point"

    max_amp = float(max(np.nanmax(x), np.nanmax(y), 1.0))
    ax.plot([0, max_amp], [0, max_amp], color="red", lw=0.8, alpha=0.8)
    ax.set_xlim(0, max_amp)
    ax.set_ylim(0, max_amp)
    ax.set_xlabel("Previous pulse amplitude (raw)")
    ax.set_ylabel("Current pulse amplitude (raw)")
    ax.set_title(f"PSA {signal_label} {title_suffix}", fontsize=9)
    ax.grid(True, alpha=0.25)
...
def plot_prpd(chara_df: pd.DataFrame, plot_dir: Path, fig_dir: Path) -> None:
    ...
    for signal_group in SIGNAL_ORDER:
        ...
        for minute, events in signal_events.groupby("minute", sort=True):
            ...
            psa_table = make_psa_pair_table(events)
            if not psa_table.empty:
                psa_base_name = f"03_psa_{signal_key}_{minute_tag}"
                ...
                render_psa_figure(psa_table, signal_label, minute_ts.strftime("%Y-%m-%d %H:%M"), psa_fig_base)
                psa_index_rows.append(
                    {
                        "signal_key": signal_key,
                        "signal_label": signal_label,
                        "minute": minute_ts.strftime("%Y-%m-%d %H:%M:%S"),
                        "pair_count": len(psa_table),
...
def plot_fft_and_main_freqs(files: list[CapFile], plot_dir: Path, fig_dir: Path) -> None:
    ...
    if not wave_df.empty:
        wave_df = wave_df[wave_df["channel"].astype(str) == UHF_FFT_CHANNEL].copy()
    if wave_df.empty:
        return
